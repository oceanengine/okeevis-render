...
function increaseArea(rect1: BBox, rect2: BBox): number {
  ...
  if (p1_x > p4_x || p2_x < p3_x || p1_y > p4_y || p2_y < p3_y) {
    // 不重叠
    increased = totalArea - (r1Area + r21Area);
    return increased;
  }
  const Len = Math.min(p2_x, p4_x) - Math.max(p1_x, p3_x);
  const Wid = Math.min(p2_y, p4_y) - Math.max(p1_y, p3_y);
  // 重叠面积是Len * Wid
  increased = totalArea - (r1Area + r21Area) + Len * Wid;
  return increased;
}
