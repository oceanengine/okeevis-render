...
export function getDomContentSize(dom: HTMLElement): [number, number] {
  ...
  const { paddingTop, paddingLeft, paddingRight, paddingBottom, width, height } = getComputedStyle(
    dom,
  );
  const contentWidth =
    (lodash.isNumber(dom.clientWidth) ? dom.clientWidth : parseInt(width, 10)) -
    parseFloat(paddingLeft || '0') -
    parseFloat(paddingRight || '0');
  const contentHeight =
    (lodash.isNumber(dom.clientHeight) ? dom.clientHeight : parseInt(height, 10)) -
