...
const interpolatePath: InterpolateFunction<Path2D> = (
  from: Path2D,
  to: Path2D,
  k: number,
): Path2D => {
  const current = to.getPathList().map((pathAction, index) => {
    ...
    if (!oldPath || oldPath.action !== pathAction.action) {
      return pathAction;
    }
      ...
      return {
        ...
        params: pathAction.params.map((param, pIndex) => {
          if (typeof param === 'number') {
            return interpolate(oldPath.params[pIndex], pathAction.params[pIndex], k);
          }
          return param;
        }),
      };
