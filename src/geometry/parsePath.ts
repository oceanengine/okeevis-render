...
export default function parsePathString(inputPath: string): PathRecord[] {
    ...
    function pushStack(): void {
        if (currentCommand) {
            ...
            if (
                pathStack.length === 0 &&
                currentCommand.toLocaleLowerCase() !== svgPathCommand.m
            ) {
            ...
            expectParamSize =
                svgCommandParamSize[currentCommand.toLocaleLowerCase()];
    ...
    return recordList.map((record: PathRecord) => {
        ...
        switch (action) {
            ...
            case svgPathCommand.S:
                canvasAPIName = canvasAPI.BEZIER_CURVE_TO;
                const [Sx1, Sy1] = getSymmetricPoint(
                    prevBezierCurveX2,
                    prevBezierCurveY2,
                    endX,
                    endY,
